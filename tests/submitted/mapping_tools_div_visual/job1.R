...
library(dplyr);
library(tidyr);
x <- read.table("/home/rvlab/jobs2/demo@gmail.com/job1/osd2014-16s-formated.csv", header = TRUE, sep=",",row.names=1);
coords <- read.table("/home/rvlab/jobs2/demo@gmail.com/job1/16S-ODV-input-corrected-dec15-formatted-coords.csv",header = TRUE, sep=",",row.names=1);
colnames(coords) <- gsub("\\.","_",gsub(" ","_",colnames(coords)));
colnames(coords)[1] <- "Longitude";
colnames(coords)[2] <- "Latitude";
indices <- read.table("/home/rvlab/jobs2/demo@gmail.com/job1/16S-ODV-input-corrected-dec15-formatted.csv",header = TRUE, sep=",",row.names=1);
x <- t(x);

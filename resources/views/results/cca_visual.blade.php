...
<script>
function dashboard(id, fData,bidata,bidatalabels){
    var barColor = 'white';
	...
	hG.init_stackedbar();
	    ...
	    // refresh MDS plot
            ...
            var mdp=fData.map(function(d){return [d.Samples,d.MDS.CCA1,d.MDS.CCA2];});
    ...
    function MDS(mD,biplotmds,biplotlabelsmds){
    	...
    	var mdssvg = d3.select(id).append("svg")
    		.attr("width", mDSDim.w + mDSDim.l + mDSDim.r+500)
	...
	//Arrowhead(s)
	mdssvg.append("defs").append("marker")
			.attr("id", "arrowhead")
			.attr("viewBox", "0 -5 10 10")
			//.attr("refX", 0.9045107*3) /*must be smarter way to calculate shift*/
			//.attr("refY", 0.06964849*3)
			.attr("markerWidth", 10)
    ...
    //Dashed lines on plot
	mdssvg.append("line")          // attach a line
			.attr("stroke-width", 0.5)
			.style("stroke", "black")  // colour the line
			.style("stroke-dasharray", ("3, 3"))
			.attr("x1", xScale(0))     // x position of the first end of the line
			.attr("y1", yScale(d3.min(mD, yValue)-2))      // y position of the first end of the line
			.attr("x2", xScale(0))     // x position of the second end of the line
			.attr("y2", yScale(d3.max(mD, yValue)+2))    // y position of the second end of the line
	...
	mdssvg.append("line")          // attach a line
				...
				.style("stroke", "black")  // colour the line
				.style("stroke-dasharray", ("3, 3"))
				.attr("x1", xScale(d3.min(mD, xValue)-2))     // x position of the first end of the line
				.attr("y1", yScale(0))      // y position of the first end of the line
				.attr("x2", xScale(d3.max(mD, xValue)+2))     // x position of the second end of the line
				.attr("y2", yScale(0))    // y position of the second end of the line
		//.attr("marker-end", "url(#arrowhead)");



   for ( var x=0; x<biplotmds.length; x++)
    {

    //Text on arrows
		mdssvg.append("text")
		.attr("x",xScale(biplotmds[x][0]*3.2))//function(d){return [d.biplotmds.fact.CCA1];}*3.2))//biplotmds.fact.CCA1)//xScale(biplotmds.fact))//
		.attr("y",yScale(biplotmds[x][1]*3))
		.text(biplotlabelsmds[x]);
	...
	mdssvg.append("line")          // attach a line
		...
		.attr("x1", xScale(0))     // x position of the first end of the line
		.attr("y1", yScale(0))      // y position of the first end of the line
		.attr("x2", xScale(biplotmds[x][0]*3))     // x position of the second end of the line
		.attr("y2", yScale(biplotmds[x][1]*3))    // y position of the second end of the line
		.attr("marker-end", "url(#arrowhead)");
		//.attr("stroke","black");
		 //.attr(marker-end="url(#arrowhead)" stroke="black" stroke-width="10")
	 }


    	// create function to update the MDS plot.
     	//md.update = function(mD, sample,biplotmds){ adding biplot data
    ...
    var mDS = fData.map(function(d){return [d.Samples,d.MDS.CCA1,d.MDS.CCA2];});
    var biplot = bidata.map(function(d){return [d.fact.CCA1,d.fact.CCA2];});
	var biplotlabels = bidatalabels.map(function(d){return [d.fact];});
    ...
    md = MDS(mDS,biplot,biplotlabels); //create the MDS plot add biplot data.
...
<script>
dashboard('#dashboard',freqData,biplot,biplotLabels);

...
<script>
function dashboard(id, fData,legenddata,indicesdata,indiceslabeldata){
    var barColor = 'white';
    ...
    function mousedblclick(d){    // utility function to be called on mousedblclick.
	    ...
	    // refresh MDS plot
            ...
            var mdp=fData.map(function(d){return [d.Samples,d.MDS.Longitude,d.MDS.Latitude];});
            
            var leg2=legenddata.map(function(d){return [d.fact.Up,d.fact.Down];});
            var ind=indicesdata.map(function(d){return [d.fact];});
            var indlab=indiceslabeldata.map(function(d){return [d.fact];});
            d3.selectAll(".dot").remove();

            

            md.update(mdp,null,leg2,ind);
	 	}
    ...
    // function to handle legend 2.
	    function legend2(lD){
	        ...
	        // create one row per segment.
	        var tr = legend.append("tbody").selectAll("tr").data(lD).enter().append("tr");
			//var colorset6=["#FFFF00", "#FFCC00", "#FF9900", "#FF6600", "#FF3300", "#FF0000"]
			var color = d3.scale.ordinal()
    		.range(["#FFFF00", "#FFCC00", "#FF9900", "#FF6600", "#FF3300", "#FF0000"]);
	        // create the first column for each segment.
    ...
    function MDS(mD,legendmds,indicesmds){
    	var md={}, mDSDim ={t: 0, r: 40, b: 50, l: 50};
    	mDSDim.w = 976,//550 - mDSDim.l - mDSDim.r+400,
    	mDSDim.h =  432;//550 - mDSDim.t - mDSDim.b;

    	// setup MDS1
    	var xValue = function(d) { return d[1];}; // data -> value
    	var xValue2 = function(d) { return d[0];}; // data -> value
    	...
    	var yValue2 = function(d) { return d[1];}; // data -> value
	...
	//Adding Image to svg
 	var img = mdssvg.append("svg:image")
    .attr("xlink:href", "{{ asset('images/World_3.jpeg') }}")
     //.attr("width", mDSDim.w)
     //.attr("height", mDSDim.h+200)
     .attr("width", 976)
     .attr("height", 432)
     .attr("x", 0)
     .attr("y",0);
    	...
    	//xScale.domain([d3.min(mD, xValue)-2, d3.max(mD, xValue)+2]);
    	//yScale.domain([d3.min(mD, yValue)-2, d3.max(mD, yValue)+2]);
   	...
   	mdssvg.append("g")
    	...
    	// create function to update the MDS plot.
     	md.update = function(mD, sample,legendmds,indicesmds){ //adding legend data
		...
		if(sample==null){
			// draw dots
                ...
                .style("fill", function(d) {
                        if ( d <= legendmds[0][1] ) {return colorset6[0]}
					        else if	( d <= legendmds[1][1] ){ return colorset6[1] }
					        else if	( d <= legendmds[2][1] ){ return colorset6[2] }
					        else if	( d <= legendmds[3][1] ){ return colorset6[3] }
					        else if	( d <= legendmds[4][1] ){ return colorset6[4] }
					        else{ return colorset6[5]}					
                            ;})
                .data(mD)                     
    	...
    	md.update(mD,null,legendmds,indicesmds);
    	//md.update(mD,null);
    	return md;
    }
    ...
    var mDS = fData.map(function(d){return [d.Samples,d.MDS.Longitude,d.MDS.Latitude];});
    var legendDiv = legenddata.map(function(d){return [d.fact.Up,d.fact.Down];});
	var indices = indicesdata.map(function(d){return [d.fact];});
	var indiceslabel = indiceslabeldata.map(function(d){return [d.fact];});


    var hG,pC,leg,md;
    hG = histoGram(sF,indices); // create the histogram.
    ...
    md = MDS(mDS,legendDiv,indices); //create the MDS plot add legend data.
    d3.select(id).append("text").attr("id","statustext").html("<table cellpadding=0 cellspacing=0><tr><td><font size=2 face='verdana' color='red'>Taxonomic index: </font><font size=2 face='verdana'>"+indiceslabel+"</font></td></table>"); //create text
    leg2= legend2(legendDiv);  // create the legend.
...
<script>
dashboard('#dashboard',freqData,legendDiv,indices,indiceslabel);

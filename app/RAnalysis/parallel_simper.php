...
class parallel_simper extends BaseAnalysis implements RAnalysis
{
    ...
    protected function buildBashScript()
    {
        ...
        fwrite($fh2, "date\n");
        fwrite($fh2, "mpiexec /usr/bin/Rscript $this->remote_job_folder/$this->job_id.R $this->remote_job_folder/$this->box $this->transpose $this->remote_job_folder/$this->box2 $this->column_select $this->remote_job_folder/ $this->permutations $this->trace > $this->remote_job_folder/cmd_line_output.txt\n");
        fwrite($fh2, "date\n");
        fwrite($fh2, "exit 0");
        fclose($fh2);
    }

...
/**
 * Executes a parallel_permanova analysis
 *
...
class parallel_permanova extends BaseAnalysis implements RAnalysis {
    ...
    /**
     * The validation rules for parallel_permanova submission form
     *
     * @var array
     */
    private $formValidationRules = [
        ...
        'single_or_multi'   =>  'required|string|in:single,multi'
    ];
    ...
    private function getInputParams()
    {
        ...
        if ($this->form['single_or_multi'] == 'single') {
            $this->single_or_multi = 1;
        } else {
            $this->single_or_multi = 2;
        }
        $this->params .= ";single_or_multi:".$this->form['single_or_multi'];
    }
    ...
    private function buildRScript()
    {
        ...
        fwrite($fh2, "date\n");
        fwrite($fh2, "mpiexec /usr/bin/Rscript $this->remote_job_folder/$this->job_id.R $this->remote_job_folder/$this->box $this->transpose $this->remote_job_folder/$this->box2 $this->single_or_multi $this->column_select $this->column_select2 $this->remote_job_folder/ $this->permutations $this->method_select > $this->remote_job_folder/cmd_line_output.txt\n");
        fwrite($fh2, "date\n");
        fwrite($fh2, "exit 0");
        fclose($fh2);
    }
